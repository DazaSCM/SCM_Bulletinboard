  ...
  <div>
    <form v-if="isCreate" v-on:submit.prevent="confirmUser" class="post-form">
      ...
      <div class="d-flex justify-content-around">
        ...
        <router-link :to='{name: "PostDetails", query: {id: this.post.id}}' class="post-cancel-btn">Cancel</router-link>
      </div>
    </form>
...
export default {
  ...
  methods: {
    ...
    addPost(){
      var uri = 'http://localhost:3000/';
      if (localStorage.getItem("user_type") == 0){
        uri = uri+'admin_update/'+this.$route.query.id;
      }
      else {
        uri = uri+'users/'+this.$route.query.user_id+'/posts/'+this.$route.query.id;
      }
      this.axios.put(uri, this.post, this.api_header).then((response) => {
        console.log(response.data);
        this.$router.push({name: 'PostDetails', query: { id: this.$route.query.id }});
      });
    }
  }
}
